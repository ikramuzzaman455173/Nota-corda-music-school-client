...
const ClassCard = ({ singleClass }) => {
  ...
  const handleSelectClass = (id) => {
    const SelectClassInfo = { selectClassId: id, class_name, image, class_level, price, class_duration,instructor_name,email:user?.email,instructor_email:email,payment:false}
    console.log('SelectClassInfo',SelectClassInfo);
    if (user && user.email) {
    ...
    >
      ...
      <div className="flex pl-6 gap-2 mb-2 flex-col">
        <p className="text-xl font-bold dark:text-white">
          Instructor Name: <span className="dark:text-warning text-info">
            {instructor_name}
          </span>
        </p>
        <p className="text-xl font-bold dark:text-white">
          available seats: <span className="dark:font-Merienda font-Pt text-warning text-left dark:text-info tracking-wider">{available_seats}</span>
        </p>
        <p className="text-xl font-bold dark:text-white">
        ...
        <p className="text-xl font-bold dark:text-white">
          Class Level: <span className="dark:font-Merienda font-Pt text-warning dark:text-info tracking-wider">{class_level}</span>
        </p>
      </div>

      <div onClick={() => handleSelectClass(_id)} className="awesome-btn text-center p-2 rounded-md my-5 cursor-pointer w-3/4 mx-auto text-green-500 font-bold">
          Selected
        </div>
